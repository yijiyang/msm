...
def generate_template(data, prefix):
    for (k, v) in data.items():
        if 'items' not in v or (is_intop(v) and len(filter_intop(v['items'])) == 0):
            yield '%s-%s' % (prefix, k)
        if 'items' in v:
            yield from generate_template(v['items'], prefix + '-' + k)
...
with open('soc.yaml', "r") as file:
    ...
    with open('_soc.template', 'w') as out:
        out.write(':\n'.join(sorted(generate_template(data, 'status'))))
        out.write(':\n')
...
with open('pmic.yaml', "r") as file:
    ...
    with open('_pmic.template', 'w') as out:
        out.write(':\n'.join(sorted(generate_template(data, 'pmic'))))
        out.write(':\n')
